...
function searchAnimationRunner() {
    ...
    const key = Object.keys(searchAnimation).reduce(
        (best, timestamp) => (timestamp <= duration ? timestamp : best),
        undefined,
    );
    if (key) {
        searchAnimation[key]();
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if ('q' in query) {
        ...
        newState({
            state: 'Feeding',
            question: formatQuestion(query['q']),
            videoFragment: `fragments/${fragment}.mp4`,
        });
    } else {
        newState({ state: 'Question' });
    }
